...
imageFat,coord = MeasureFatThickness(image,image_path)
img_finale = ajout_box(imageFat,coord)
cv2.imshow("img finale",img_finale)
cv2.waitKey(0)  # Attendre qu'une touche soit pressée pour fermer la fenêtre
cv2.destroyAllWindows

"""MFT = threading.Thread(target=MeasureFatThickness,args=(image,image_path,))
#T2 = threading.Thread(target=aditionneur,args=(5,47,))
